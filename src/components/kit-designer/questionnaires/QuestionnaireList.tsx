...
const useQuestionnaireLogic = () => {
  ...
  const debouncedHandleReorder = debounce(
    async (newOrder: any[], questionnaireId: TId) => {
      try {
...
const DescriptionSection = ({
  ...
  editMode ? (
    <MultiLangTextField
      ...
      onChange={handleChange}
      translationValue={
        langCode ? (tempValues.translations?.[langCode]?.description ?? "") : ""
      }
  ...
  >
    ...
    <Box
      ...
      sx={{
        ...
        borderRadius: "50%",
        backgroundColor:
          item.questionsCount === 0 ? "error.main" : "background.variant",
        color:
          item.questionsCount === 0 ? "error.contrastText" : "text.primary",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
      }}
