...
const FlatGaugeComponent: React.FC<Props> = ({
  ...
  // تنظیمات حالت vertical-trapezoid
  const topWidth = 24;
  const cellHeight = 20;
  const slope = 0.1; // 10% کاهش عرض

  return (
    <Box
      sx={{
        ...styles[
          position === "vertical" || position === "vertical-trapezoid"
            ? "centerCH"
            : "centerH"
          ],
        userSelect: "none",
        ...
        {position === "vertical-trapezoid" ? (
          ...
          >
            {Array.from({ length: levels }).map((_, i) => {
              // برعکس: خانه بالا بزرگترین، خانه پایین کوچکترین
              ...
              const clipPath = `polygon(${offsetTop}px 0, ${offsetTop + widthTop}px 0, ${offsetBottom + widthBottom}px ${cellHeight}px, ${offsetBottom}px ${cellHeight}px)`;
